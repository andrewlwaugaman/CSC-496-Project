...
parser.add_argument("--num-candidates", dest="candidates", default=3, type=int)
...
if args.max_ranking_length == -1:
    max_ranking_length = args.candidates
else:
...
possible_rankings = []
for j in range(min_ranking_length, max_ranking_length + 1):
    possible_rankings.extend(itertools.permutations(range(args.candidates), j))
...
for i in range(args.elections):
    ...
    for _ in range(max_rankings):
        rankings.add(possible_rankings[random.randrange(0, len(possible_rankings))])
    vote_partitions = []
